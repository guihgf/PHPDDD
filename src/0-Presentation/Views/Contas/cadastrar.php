...
<!-- Default box -->
<form id="form" method="POST" action="<?=isset($conta["id"])?"alterar":"cadastrar"?>" onsubmit="return false;">
	<?php
		if(isset($conta["id"])){
	?>
		<input type="hidden" name="codigo" value="<?=$conta["id"]?>"/>
			...
			<div class="box-body">
				<div class="col-md-4">
					<div class="form-group">
						...
						<input type="text" name="nome" class="form-control" id="nome" value="<?=(isset($conta["nome"])?$conta["nome"]:"")?>">
					</div>
				</div>
				<div class="col-md-3">
					<div class="form-group">
						<label for="principal">Conta Principal?</label>
						<select name="padrao" id="padrao" class="form-control">
							<option value="1" <?=(((isset($conta["padrao"])&&$conta["padrao"]=="1") || !isset($conta["padrao"]))?"selected":"")?> >Não</option>
							<option value="2" <?=((isset($conta["padrao"])&&$conta["padrao"]=="2")?"selected":"")?>>Sim</option>
						</select>
					</div>
				</div>
			</div>
